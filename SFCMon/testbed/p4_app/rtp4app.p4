...
control MyIngress(inout headers hdr,
    ...
    /*action nsh_tunnel_forward(egressSpec_t port) {
        standard_metadata.egress_spec = port;
        hdr.ipv4.ttl = hdr.ipv4.ttl - 1;
    }*/
                    ...
                    if (srcPort != 0) {
                        ...
                        if (meta.rtp4mon_m.ctr_packets >= THRESHOLD_MIN_PKTS) {
                            ...
                            if (meta.rtp4mon_m.is_hh == 1) {
                                create_flow_id_bf(meta.rtp4mon_m,hdr.ipv4.srcAddr,hdr.ipv4.srcAddr,srcPort,dstPort,hdr.ipv4.protocol,hdr.nsh_sp.spi,hdr.nsh_sp.si);
                                membership_query_bf(meta.rtp4mon_m);
                                if (meta.rtp4mon_m.is_stored == 0) {
                                    add_iblt_entry(meta.rtp4mon_m);
                                }
